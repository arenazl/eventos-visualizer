...
const LocationBar: React.FC<LocationBarProps> = ({ onLocationChange, onStyleChange }) => {
    ...
    if ('geolocation' in navigator) {
      navigator.geolocation.getCurrentPosition(
        async (position) => {
          ...
          try {
            const response = await fetch(`${import.meta.env.VITE_API_URL || ''}/api/location/nearest`, {
              method: 'POST',
  ...
  const fallbackToIPLocation = async () => {
    try {
      const response = await fetch(`${import.meta.env.VITE_API_URL || ''}/api/location/detect`)
      const data = await response.json()
