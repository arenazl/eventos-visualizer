...
# -*- coding: utf-8 -*-
"""
Agrega imágenes de Ticketmaster API a eventos de Latinoamérica
...
def get_ticketmaster_image(event_title, city, country_code):
    ...
    search_term = event_title.split('-')[0].split('|')[0].strip()

    params = {
        'apikey': TICKETMASTER_KEY,
        'keyword': search_term[:50],  # Limitar longitud
        'city': city,
...
def process_json_file(file_path):
    ...
    country = data.get('country', data.get('pais', ''))

    # Mapeo de países a códigos
    country_codes = {
        'Argentina': 'AR', 'Brasil': 'BR', 'Chile': 'CL',
        'Colombia': 'CO', 'México': 'MX', 'Perú': 'PE',
        'Uruguay': 'UY', 'Venezuela': 'VE', 'Ecuador': 'EC',
        'Bolivia': 'BO', 'Paraguay': 'PY', 'Panama': 'PA',
        'Costa Rica': 'CR', 'Puerto Rico': 'PR'
    }

    country_code = country_codes.get(country, 'AR')
    ...
    for i, evento in enumerate(eventos):
        ...
        source = evento.get('source', evento.get('fuente', ''))
        if 'ticketmaster' not in source.lower():
            continue
        ...
        image_url = get_ticketmaster_image(title, city, country_code)
...
def main():
    print('='*70)
    print('🖼️  AGREGANDO IMÁGENES DE TICKETMASTER - LATINOAMÉRICA')
    print('='*70)

    total_added = 0
    total_files = 0

    # Recorrer todas las subcarpetas (sudamerica, centroamerica, etc.)
    for subregion_dir in LATAM_DIR.iterdir():
        if not subregion_dir.is_dir():
            continue

        print(f'\n🌎 {subregion_dir.name.upper()}')
        print('-'*70)

        # Recorrer países
        for country_dir in subregion_dir.rglob('*.json'):
            added = process_json_file(country_dir)
            total_added += added
            total_files += 1
